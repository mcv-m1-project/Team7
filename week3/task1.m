...
num_image = 0;                      % Number of image to display text on screen

% Load mask folder
sdir = 'mask_results_test';
samples = dir(sdir); 
...
for ii=1:total_images    
    ...
    for i=1:length(shape_mesarurements)
      ...
      if((area > area_min) && ((ratio) > ratio_min))
          ...
          % show_bounding_boxes(image, boundingBox);
...
save('matlab_files/bounding_boxes_list.mat', 'bounding_boxes_list', '-v7.3');
...
% Input: image, bounding box and shape of signal
% Output: None
function show_bounding_boxes(image, boundingBox)
