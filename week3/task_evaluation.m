...
function task_evaluation()
% Show description on screen about evaluation task
show_description();
% User menu to choose dataset 
[dataset, ~] = choose_dataset();
[method, mat_file, ~] = choose_method();

% Load directory of matlab files of method selected
sdir_method = strcat('matlab_files/', method, '/', mat_file);
sdir_method = strcat(sdir_method, dataset, '.mat');
bounding_boxes = load(sdir_method);
bounding_boxes = bounding_boxes.bounding_boxes_list;

% Generate mask images with bounding boxes
% 1. Load improved mask folder of dataset
% 2. Load bounding boxes
% 3. Put 0 all pixels that not belongs to any bounding box
sdir_mask = strcat('improved_mask_', dataset);
samples = dir(sdir_mask);
...
for ii=1:total_images  
    ...
    directory = sprintf('%s/%s.png', sdir_mask, name_sample);
    ...
    bb = bounding_boxes.(bb_name);   
    [total_detections, ~] = size(bb);
    
    % Print detections on image
    for jj=1:total_detections
        % While bounding boxes is not 0 (detection discarded)
...
[~, name_sample, ~] = fileparts(samples(ii).name);
dir_mask = sprintf('%s/mask/mask.%s.png', sdir_method, name_sample);
...
% save_dir = strcat(sdir_method);
% save(sdir_method, 'bounding_boxes_list', '-v7.3');
% disp('Save results.mat: done');
% save('task4(): done');
end
...
% Output: None
function show_description()
disp('------------------ TASK EVALUATION DESCRIPTION -------------------');
...
% Input: None
% Output: dataset, valid_option
function [dataset, valid_option] = choose_dataset()
valid_option = 0;
disp('Dataset train selected');
prompt = 'Do you evaluate train or validation split? [train/validation] : ';
